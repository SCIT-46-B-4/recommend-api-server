...
def apply_itinerary_pattern(df_ranked_dest, df_final_base, user_info):
    ...
    for day in unique_days:
        ...
        # user_id별로 그룹화하여 schedule_style 적용
        for user_id, df_user_day in df_day.groupby("user_id"):
            # 해당 user_id의 schedule_style 가져오기 (기본값 5)
            ...
            if day_index == 1:
                day_key = "first_day"
            elif day_index == len(df_ranked_dest["day"].unique()):
                day_key = "last_day"
            else:
                day_key = "middle_day"
            ...
            # 장소 분류 (type 값 기반)
            # 🔹 type == 2 (식당) 요소를 content 기준으로 분류

            categorized = classify_df_user_day_by_type(df_user_day)

            # Type 2에 대한 분류 - Content : 음식점 / 카페 / 이자카야
            # Type 2에 대한 요소들이 들어있는 Dict.
            classified = map_food_type_slots(categorized, food_type_order, user_schedule_style, day_key, df_user_day)
            ...
            # TODO : Data를 확인한 결과 중복되는 추천 여행지가 있을 경우 안들어가는 현상 발생.
            # TODO : 만약 근처의 관광지가 없을 경우에 추가적인 처리를 어떤식으로 할것인가?
            # 기록: used_places 업데이트 (여기서 각 candidate의 dest_id 사용)
            for cand in itinerary_list:
                if cand is not None:
                    used_places.add(cand["dest_id"])
            if day not in final_schedule:
                final_schedule[day] = {}
            final_schedule[day][user_id] = itinerary_list

            if day not in final_schedule:
                final_schedule[day] = {}

            final_schedule[day][user_id] = itinerary_list  # user_id별 일정 저장
...
# DataFrame -> JSON 변환 후 저장
final_output_path = data_dir

# 일정 적용 실행
final_schedule = apply_itinerary_pattern(df_ranked_dest, df_final_base, user_info)
print(tabulate(final_schedule, headers="keys"))

# JSON 저장 (day별 최종 일정 저장)
for day, users_schedule in final_schedule.items():
    for user_id, schedule in users_schedule.items():
        file_name = f"final_recommendation_{day}_{user_id}.json"
        file_path = os.path.join(final_output_path, file_name)

        with open(file_path, "w", encoding="utf-8") as json_file:
            json.dump(schedule, json_file, ensure_ascii=False, indent=4)

        print(f"✅ 일정 저장 완료: {file_path}")
